...
fn get_parameters(args: &Args) -> io::Result<Parameters> {
    ...
    let wgpu_rev = if args.auto {
        let remote = config.wgpu.updatream_remote.unwrap_or_else(|| "upstream".to_string());
...
fn preamble(params: &Parameters) -> io::Result<()> {
    ...
    shell(&params.gecko_path, "hg", &["diff"]);
    commit(params, "(Don't land) Uncommited changes before the wgpu update.", None);

    shell(&params.gecko_path, "./mach", &["vendor", "rust"]);
...
fn vet_changes(params: &Parameters, deltas: &[Delta]) -> io::Result<()> {
    for delta in deltas {
        ...
        shell(&params.gecko_path, "./mach", &["cargo", "vet", "certify", crate_name, &prev, &next, "--criteria", "safe-to-deploy"]);
    }
    ...
    shell(&params.gecko_path, "./mach", &["cargo", "vet"]);
